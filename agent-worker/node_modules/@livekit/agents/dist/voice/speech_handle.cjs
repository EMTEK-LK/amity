...
__export(speech_handle_exports, {
  ...
  SpeechHandleCircularWaitError: () => SpeechHandleCircularWaitError,
...
class SpeechHandle {
  ...
  async waitIfNotInterrupted(aw) {
    const allTasksPromise = import_throws.ThrowsPromise.all(aw);
    const fs = [allTasksPromise, this.interruptFut.await];
    await import_throws.ThrowsPromise.race(fs);
  }
