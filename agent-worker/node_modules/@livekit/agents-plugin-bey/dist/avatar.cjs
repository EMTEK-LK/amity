...
class BeyException extends Error {
  constructor(message) {
    super(message);
...
class AvatarSession extends import_agents.voice.AvatarSession {
  ...
  constructor(options = {}) {
    ...
    if (!this.apiKey) {
      throw new BeyException(
        "The api_key must be set either by passing apiKey to the client or by setting the BEY_API_KEY environment variable"
      );
    }
    ...
    this.connOptions = options.connOptions || import_agents.DEFAULT_API_CONNECT_OPTIONS;
  }
  /**
  ...
  async start(agentSession, room, options = {}) {
    ...
    try {
      ...
      localParticipantIdentity = ((_a = jobCtx.job.participant) == null ? void 0 : _a.identity) || "";
    ...
    const at = new import_livekit_server_sdk.AccessToken(livekitApiKey, livekitApiSecret, {
  ...
  async startAgent(livekitUrl, livekitToken) {
    for (let i = 0; i < this.connOptions.maxRetry; i++) {
      try {
        ...
        if (e instanceof import_agents.APIConnectionError) {
        ...
        if (i < this.connOptions.maxRetry - 1) {
          await new Promise(
            (resolve) => setTimeout(resolve, (0, import_agents.intervalForRetry)(this.connOptions, i))
          );
        }
      }
    }
